...
from maskrcnn_benchmark.data import make_data_loader_val, make_batch_data_sampler, build_dataset, get_dataset_statistic,\
    make_data_loader, make_data_sampler
...
from maskrcnn_benchmark.data import get_dataset_statistics, get_dataset_statistic
...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.use_loss = config.MODEL.ROI_RELATION_HEAD.USE_EXTRA_LOSS
        # load parameters
        ...
        # load class dict
        statistics = get_dataset_statistic(config)
        print('cfg.SOLVER.EPSILON:',config.SOLVER.EPSILON)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        prod_reps = [] # joint relation embedding
        pair_preds = [] # pair-wise label
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.rel_compress_fg = nn.Linear(self.num_rel_cls, self.num_rel_cls, bias=True)

        # initialize layer parameters
        layer_init(self.post_emb, 10.0 * (1.0 / self.hidden_dim) ** 0.5, normal=True)
        layer_init(self.post_cat, xavier=True)
        layer_init(self.rel_compress, xavier=True)
        layer_init(self.rel_compress_fg, xavier=True)
