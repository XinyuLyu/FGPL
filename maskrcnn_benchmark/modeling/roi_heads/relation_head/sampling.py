...
class RelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets, times_bg=None):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            #remove zero shot predicates
            # tgt_zs_idxs = torch.nonzero( tgt_rel_labs < 99, as_tuple=True)[0]
            # tgt_rel_labs = tgt_rel_labs[tgt_zs_idxs]
            # tgt_pair_idxs = tgt_pair_idxs[tgt_zs_idxs]
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility):
        ...
        times_bg = None
        num_fg = fg_rel_triplets.shape[0]
        num_bg = self.batch_size_per_image - num_fg
        if times_bg is not None:
            num_bg = min(num_bg, int(times_bg * num_fg))
        num_bg = min(num_bg, bg_rel_triplets.shape[0])

        if bg_rel_triplets.shape[0] > 0:
            perm = torch.randperm(bg_rel_triplets.shape[0], device=device)[:num_bg]
