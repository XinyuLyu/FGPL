...
from maskrcnn_benchmark.data.samplers.grouped_batch_sampler import get_eps, GroupedBatchSampler
from tqdm import tqdm
import random
import queue
from maskrcnn_benchmark.data.transforms.transforms import get_epslion, Normalize, ToTensor
...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                 ...
                 with_clean_classifier=False, cfg=None):
        """
    ...
    def get_statistics(self, cfg=None):
        fg_matrix, bg_matrix, cfg = get_VG_statistics(img_dir=self.img_dir, roidb_file=self.roidb_file,
    ...
    def get_statistic(self, cfg=None):
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True, cfg=None):
    ...
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file,
                           ...
                           filter_duplicate_rels=False, cfg=cfg)
    ...
    for ex_ind in tqdm(range(len(train_data))):
        ...
        for (o1, o2) in o1o2_total:
            bg_matrix[o1, o2] += 1
    cfg = get_eps(cfg)
    return fg_matrix, bg_matrix, cfg
...
def load_info(dict_file, add_bg=True):
    ...
    """
    info = json.load(open(os.path.join(dict_file), 'r'))
    if add_bg:
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap,
                ind_to_predicates=None, img_info=None, cfg=None):
    """
    ...
    fg_matrix = np.load("/home/lvxinyu/lib/scene-graph-benchmark/misc/fg_matrix_overlap.npy")
    fg_matrix[0, :, :] = 0
    fg_matrix[:, 0, :] = 0
    fg_matrix[:, :, 0] = 0
    fg_matrix[0, 0, 0] = 1
...
def get_VG_statistic(img_dir, roidb_file, dict_file, image_file, must_overlap=True, cfg=None):
    print("!!!!!!!!!!!!!!Get Visual Genome Statistics!!!!!!!!!!!!!!!!!!")
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file,
                           dict_file=dict_file, image_file=image_file, num_val_im=5000,
                           filter_duplicate_rels=False, cfg=cfg)
