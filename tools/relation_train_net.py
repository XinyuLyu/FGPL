...
def train(cfg, local_rank, distributed, logger):
    ...
    # cm = np.load('/home/lvxinyu/lib/scene-graph-benchmark/misc/conf_mat_transformer_train.npy')
    logger.info("cfg.SOLVER.EPSILON:\n{}".format(cfg.SOLVER.EPSILON))
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        # loss_dict, relation_loss = model(images=images, targets=targets, iteration=iteration, cm=cm)  # relation_loss[1] contrastive loss
        loss_dict, relation_loss = model(images=images, targets=targets, iteration=iteration)
        ...
        # reduce losses over all GPUs for logging purposes
        if (cfg.MODEL.ROI_RELATION_HEAD.USE_CONTRA_LOSS or cfg.MODEL.ROI_RELATION_HEAD.USE_DISTANCE_LOSS):
        ...
        if cfg.SOLVER.TO_VAL and iteration % cfg.SOLVER.VAL_PERIOD == 0:
            ...
            val_result, cm = run_val(cfg, model, val_data_loaders, distributed, logger)
...
#         # fix_eval_modules(eval_modules)
#         if clean_classifier:
#             fix_eval_modules_no_classifier(model, with_grad_name='_clean') # finetuning classifier
#         else:
#             fix_eval_modules(eval_modules)
#
#         images = images.to(device)
